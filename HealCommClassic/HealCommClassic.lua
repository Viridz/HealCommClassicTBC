...
local frames = {
				...
				["focus"] = { bar = getglobal("FocusFrameHealthBar"), frame = _G["FocusFrame"] },
...
--[[
	Function: PLAYER_FOCUS_CHANGED
	Purpose: Update player focus heal bars
]]--
function HealCommClassic:PLAYER_FOCUS_CHANGED()
	self:UpdateFrame(frames["focus"].bar, "focus", currentHeals[UnitGUID("focus")] or 0, currentHots[UnitGUID("focus")] or 0)
end
...
function HealCommClassic:UpdateIncoming(...)
	...
	for i=1, select("#", ...) do
		...
		if UnitGUID("focus") == targetGUID then 
			self:UpdateFrame(frames["focus"].bar, "focus", amount, hotAmount)
		end
